...
<template>
  <div
    id="app-layout"
    class="flex flex-col h-screen overflow-hidden bg-slate-50"
  >
    <header
      class="app-header bg-slate-900 text-white px-6 py-4 flex justify-between items-center z-10 shadow-lg"
    >
      <div class="logo flex items-center gap-3 select-none">
        <div
          class="logo-icon bg-gradient-to-tr from-indigo-500 to-purple-500 w-10 h-10 rounded-xl flex items-center justify-center shadow-md shadow-indigo-500/20"
        >
          <i class="pi pi-graduation-cap text-white text-xl"></i>
        </div>
        <span
          class="text-xl font-bold tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent"
        >
          Електронна Кафедра
        </span>
      </div>
      ...
      <div class="role-simulator flex gap-4 items-center flex-wrap justify-end">
        <div class="flex items-center gap-2">
          <span class="status-text text-slate-400 text-sm font-medium"
            >Тестувати роль:</span
          >
          <Select
        ...
        <div
          v-if="authStore.user"
          class="flex items-center gap-3 bg-slate-800/60 border border-slate-700/50 px-4 py-2 rounded-xl"
        >
          <span
            class="text-xs text-indigo-400 font-semibold uppercase tracking-wider"
            >{{ authStore.roleLabel }}</span
          >
          <span
            class="text-sm text-slate-200 font-medium truncate max-w-[180px]"
    ...
    <div class="main-content flex flex-1 overflow-hidden">
      <aside
        class="sidebar w-72 bg-white border-r border-slate-200 p-4 flex flex-col gap-6 z-0 shadow-sm"
      >
        <div class="px-3 py-2">
          <span
